...
const ResetPasswordChange = () => {
  const { currentUser, verifyPasswordResetToken, changePassword } = useAuthContext();
  ...
  useEffect(() => {
    if (!hasRenderedOnce.current) {
      ...
      verifyToken(); // Skip the second render
    }
  }, []);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setStatus, setSubmitting }) => {
      ...
      try {
        await changePassword(currentUser?.email, values.newPassword);
        setHasErrors(false);
      } catch {
        setHasErrors(true);
