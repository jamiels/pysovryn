...
import { uploadProfileImage } from '@/services/user_services.ts';
import { getAuth } from '@/auth';
import { validateImageFile } from '@/utils/crop-image.ts';
import { showToast } from '@/utils/toast_helper.ts';

const CrudAvatarUpload = () => {
  const { auth, saveAuthToState } = useAuth();
  ...
  useEffect(() => {
    if (auth?.user?.profileImageURL) {
      setAvatar([{ dataURL: auth.user.profileImageURL }]);
    }
  }, [auth]);
  ...
  const handleUploadProfileImage = async (file: File) => {
    ...
    try {
      const is_valid_image = validateImageFile(file as File);
      if (!is_valid_image) {
        showToast('error', 'Invalid image file. Must be a maximum of 10MB Image');
        return;
      }
      const isUploaded = await uploadProfileImage(auth.user.userId, file);
      if (isUploaded) {
        setAvatar([{ dataURL: URL.createObjectURL(file) }]);
        saveAuthToState({
    ...
    <ImageInput
      ...
      onChange={(selectedAvatar) => {
        ...
        if (selectedAvatar.length > 0 && selectedAvatar[0].file) {
          handleUploadProfileImage(selectedAvatar[0].file);
        }
      }}
    >
      {({ onImageUpload }) => (
        <div className="image-input size-16" onClick={onImageUpload}>
          ...
          <div
            ...
            style={{ backgroundImage: `url(${toAbsoluteUrl(`/media/avatars/default-user.jpg`)})` }}
          >
