...
import { addProducer } from '@/services/producer_services.s.ts';
...
export const AddProducerDialog: React.FC<AddProducerDialogProps> = ({ isOpen, onClose }) => {
  const [formData, setFormData] = useState<IProducerRequest>({
    name: '',
    space_id: 0
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const status = await addProducer(formData);
    if (status === 200) {
      fetchProducers();
          ...
          <div className="flex items-center justify-between flex-wrap grow gap-5 pb-7.5 pt-7.5">
            <div className="flex flex-col justify-center gap-2">
              ...
              <div className="flex items-center gap-2 text-sm font-normal text-gray-700">
                Create and manage your producers effortlessly.
              </div>
            </div>
          </div>
        </ModalHeader>
