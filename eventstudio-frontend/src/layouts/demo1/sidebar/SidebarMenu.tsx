...
const SidebarMenu = () => {
            ...
            <MenuIcon className={clsx('items-start text-gray-500 dark:text-gray-400', iconWidth)}>
              {item.icon && (
                <img
                  className={'w-5 h-5'}
                  alt={item.icon}
                  src={toAbsoluteUrl(`/media/coloured-icons/${item.icon}`)}
                />
              )}
            </MenuIcon>
      ...
      return (
        <MenuItem key={index}>
          ...
          >
            <MenuIcon className={clsx('items-start text-gray-500 dark:text-gray-400', iconWidth)}>
              {item.icon && (
                <img alt={item.icon} src={toAbsoluteUrl(`/media/coloured-icons/${item.icon}`)} />
              )}
            </MenuIcon>
            <MenuTitle className="text-sm font-medium text-gray-800 menu-item-active:text-primary menu-link-hover:!text-primary">
              {item.title}
        ...
        >
          ...
          >
            ...
            {item.collapse ? (
              <div>
                <MenuIcon
                  className={clsx('items-start text-gray-500 dark:text-gray-400', iconWidth)}
                >
                  {item.icon && (
                    <img
                      className={'w-5 h-5'}
  ...
  const buildMenuBullet = () => {
    return (
      <MenuBullet className="flex w-[6px] relative before:absolute before:top-0 before:size-[6px] before:rounded-full rtl:before:translate-x-1/2 before:-translate-y-1/2 menu-item-active:before:bg-primary menu-item-hover:before:bg-primary -ml-0.75"></MenuBullet>
    );
  };
