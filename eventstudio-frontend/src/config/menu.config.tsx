...
const MENU_SIDEBAR: TMenuConfig = [
  {
    title: 'Dashboard',
    icon: 'dashboard.png',
    path: '/'
...
export const MenuSidebarHandler = () => {
  const { activeSpace } = useSpace();
  const { auth } = useAuth();

  if (!activeSpace) return [];

  const filterMenu = (menu: TMenuConfig): TMenuConfig => {
    return menu.filter((item) => {
      if ('heading' in item) return true;

      // Permission checks
      if (item.title === 'Users') return activeSpace.isAdmin;
      if (item.title === 'Administration') return auth?.user.role === 'EVENTSTUDIO_ADMIN';
      ...
      switch (item.title) {
        case 'Events':
          return activeSpace.pagePermits?.isEventPermitted === true;
        case 'Organizations':
          return activeSpace.pagePermits?.isOrganizationPermitted === true;
        case 'Venues':
          return activeSpace.pagePermits?.isVenuePermitted === true;
        case 'Speakers':
          return activeSpace.pagePermits?.isSpeakerPermitted === true;
        case 'Sponsorships':
          return activeSpace.pagePermits?.isSponsorPermitted === true;
        default:
          return true;
      }
    });
  ...
  return filterMenu(MENU_SIDEBAR);
};
      ...
      },
      {
        ...
        children: [
          ...
          },
          {
            title: 'Members & Roles',
            children: [
              { title: 'UpcomingEventsTable Starter', path: '/account/members/team-starter' },
